    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!Array.isArray(alphaData)) {
            ...
            alphaData.forEach((row, index) => {
                ...
                requiredFields.forEach((field) => {
                    if (!(field in row)) {
                        errors.push(`La ligne ${index + 1} dans la feuille "Alpha" est manquante le champ "${field}".`);
                    }
                });
                ...
                if (typeof row["Prénom"] !== "string" ||
                    ...
                    errors.push(`La ligne ${index + 1} dans la feuille "Alpha" a des types de données invalides pour un ou plusieurs champs.`);
                }
            });
        }
        ...
        alphaData.forEach((row, index) => {
            ...
            if (firstName && lastName && promo && majeure) {
                ...
                errors.push(`Données insuffisantes pour identifier l'étudiant à la ligne ${index + 1}. Les champs "Prénom", "Nom", "A" et "Majeure" sont requis.`);
            }
        });
        ...
        if (bulkSetOperations.length > 0) {
            try {
                ...
                errors.push(`Échec de l'écriture en masse pour les mises à jour : ${bulkError.message}`);
            }
        }
        ...
        if (bulkAddOperations.length > 0) {
            try {
                ...
                errors.push(`Échec de l'écriture en masse pour les ajouts : ${bulkError.message}`);
            }
        }
        ...
        if (bulkWriteSetResult || bulkWriteAddResult) {
            ...
            message += `Tentative de modification de ${bulkSetOperations.length + bulkAddOperations.length} document(s), mais des erreurs ont été rencontrées.`;
        }
        else {
